...
// suspiciousPatterns matches shell constructs that have no place in a credential
// helper command and most likely indicate config tampering or a typo that escaped
// quoting. The list is intentionally conservative: it blocks the highest-risk
// forms (command separators, backtick/process substitution, redirections to and
// from /dev/null lookalikes, etc.) while leaving legitimate patterns like
// `op read "op://vault/item"` untouched.
//
...
// always bypass these checks by choosing a differently-structured command.
// The goal is to surface accidental misconfigurations and alert on the most
// common injection shapes, not to sandbox the shell.
var suspiciousPatterns = []*regexp.Regexp{
	// Command separators: `;`, `&&`, `||`, newlines, null bytes.
	regexp.MustCompile(`(?i)[;|&]|\x00|\n`),
	// Backtick command substitution: `cmd`
	regexp.MustCompile("` "),
	// Redirection to/from arbitrary files: > file, 2> file, >> file, < file.
	regexp.MustCompile(`(?i)\d*>>?\s*\S|<<|<\s*\S`),
}
	...
	for _, re := range suspiciousPatterns {
		if loc := re.FindStringIndex(cmd); loc != nil {
			return fmt.Errorf(
				...
				label, loc[0], cmd[loc[0]:min(loc[1]+8, len(cmd))],
			)
		}
	}
	return nil
